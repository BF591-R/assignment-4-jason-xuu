...
caption_4 <- 'Variance explained by each PC (bars) and cumulative variance (line).'
caption_5 <- 'PC1 vs PC2 colored by subtype (SixSubtypesClassification).'
caption_8 <- 'Heatmap of DE probe intensities, RdBu palette.'
```
...
# need PC order on x axis (PC1, PC2, ...) not alphabetical
plot_tib <- variance_tibble
plot_tib$principal_components <- factor(plot_tib$principal_components, levels = plot_tib$principal_components)
ggplot(plot_tib, aes(x = principal_components)) +
